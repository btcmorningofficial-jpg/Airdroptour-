...
class ByBugInvite {
  static Future<List<Map<String, dynamic>>> getInvitableUsers({
    ...
    try {
      ...
      final resp = await http
          .get(
            Uri.parse(
              '${ByBugDB.apiBaseUrl}/db/invitable_users.php?channel_id=${Uri.encodeComponent(channelId)}&search=${Uri.encodeComponent(search)}',
            ),
            headers: headers,
          )
          .timeout(_kDefaultTimeout);
  ...
  static Future<List<dynamic>> sendChannelInvites({
    ...
    try {
      ...
      final j = _safeDecode(resp);
      if (j == null) return [0, 'Sunucudan geçersiz yanıt alındı'];
      if (j['status'] == 1) return [1, j['invited'] ?? targetUids];
  ...
  static Future<List<Map<String, dynamic>>> getNotifications() async {
    try {
      ...
      final resp = await http
          .get(
            Uri.parse('${ByBugDB.apiBaseUrl}/db/notifications.php'),
            headers: headers,
          )
          .timeout(_kDefaultTimeout);
      final decoded = _safeDecode(resp);
  ...
  static Future<List<dynamic>> respondToChannelInvite({
    ...
    try {
      ...
      final resp = await http
          .post(
            Uri.parse('${ByBugDB.apiBaseUrl}/db/notification_respond.php'),
            headers: headers,
            body: jsonEncode({
              'channel_id': channelId,
              'accept': accept,
  ...
  static Future<void> markNotificationRead(String tag) async {
    try {
      ...
      await http
          .post(
            Uri.parse('${ByBugDB.apiBaseUrl}/db/notification_read.php'),
            headers: headers,
